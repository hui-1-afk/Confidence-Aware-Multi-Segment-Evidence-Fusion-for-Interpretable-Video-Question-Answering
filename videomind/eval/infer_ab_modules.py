...
def calculate_weights(conf1, conf2, T=1.0, mode='softmax'):
    ...
    if mode == 'equal':
        ...
        return (1.0, 0.0) if conf1 >= conf2 else (0.0, 1.0)
    
    # 默认模式: Softmax
    c1_T, c2_T = conf1 / T, conf2 / T
    exp_c1_T, exp_c2_T = np.exp(c1_T), np.exp(c2_T)
    denominator = exp_c1_T + exp_c2_T
    if denominator == 0: return (0.5, 0.5)
    return (exp_c1_T / denominator).item(), (exp_c2_T / denominator).item()
...
def parse_args():
    ...
    parser.add_argument('--fusion_mode', default='softmax', choices=['softmax', 'equal', 'hard', 'top1'])
    # 在 parse_args() 中添加：
    parser.add_argument('--no_grounding', action='store_true', help='Skip grounding and use full video')
            ...
            try:
                ...
                if do_answering:
                    ...
                    if 'pred' in dump and len(dump['pred']) >= 2 and adapter_state['verifier'] and 'probs' in dump:
                        ...
                        # 计算权重并选择最终答案/时刻
                        w1, w2 = calculate_weights(conf1, conf2, T=1.0, mode=args.fusion_mode)
                        # 如果是 top1 模式，强制忽略第二个答案
                        if args.fusion_mode == 'top1':
                            final_answer, final_moment = ans1, moment1
